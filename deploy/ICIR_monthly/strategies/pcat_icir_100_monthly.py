...
前置条件：
  ...
  2. DuckDB 因子库路径由环境变量 PAPER_DATA_DIR 指定
  3. 原始回测策略文件在 COLLAB_ROOT (环境变量) 下

用法：
  PAPER_DATA_DIR=/root/lqq_bot_workspace/data \
  PAPER_COLLAB_ROOT=/root/lqq_bot_workspace \
...
OUTPUT_DIR = os.environ.get('PAPER_OUTPUT_DIR', '/root/lqq_bot_workspace/zz1000/output')
SIGNAL_FILE = os.environ.get(
    'PAPER_SIGNAL_FILE',
    os.path.join(OUTPUT_DIR, 'paper_signal_targets.json'),
)
COLLAB_ROOT = os.environ.get(
    'PAPER_COLLAB_ROOT',
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
)
STRATEGY_MODULE = os.environ.get(
    'PAPER_STRATEGY_MODULE',
    'IC5_T100_ICIRTM10_ICIRN20_月频_218因子',
)
TOP_N = int(os.environ.get('PAPER_TOP_N', '100'))
INITIAL_CAPITAL = float(os.environ.get('PAPER_CAPITAL', '100000000'))
...
def _init_quant_strategy():
    ...
    try:
        ...
        if StrategyClass is None:
            raise ImportError(f"在 {STRATEGY_MODULE} 中未找到策略类")
    except ImportError as e:
        logger.error("无法导入 quant_backtest 或策略模块 %s: %s", STRATEGY_MODULE, e)
        return None, None
